...
@Injectable()
export class UploadToS3Interceptor implements NestInterceptor {
  private s3: S3Client;
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    const uploadParams = {
      ...
      Key: `users/${req.file.filename}`, // The file name in the S3 bucket
      Body: req.file.buffer, // The file's buffer from memory
    ...
    return from(this.s3.send(command)).pipe(
      switchMap(() => next.handle()), // Proceed to next handler
      catchError((error) => {
        const errorMessage =
